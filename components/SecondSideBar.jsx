...
const SecondSideBar = ({ SideBarSecondLinks, closes }) => {
    ...
    return (
        <div className='flex flex-col gap-y-1 w-full h-full'>
            <span className={` '' ${isSidebarOpen ? 'block text-xs text-[#FFFFFFBD]' : 'block md:hidden lg:block text-xs text-[#FFFFFFBD] '
                }`}>Setting and personal </span>
            <div className='flex flex-col gap-y-4 w-full h-full'>
                ...
                <button className={` text-base w-[100%] hover:bg-[#E0F3F3] hover:text-[black] rounded-[5px] bg-[#F8F8F863] p-2 text-white capitalize hidden md:block`} onClick={openModal}>
                    <div className='flex items-center text-sm  w-[100%]  gap-x-4 capitalize'>
                ...
                <Modal isOpen={isOpenModal} close={() => setIsOpenModal(false)} height='400px'>
