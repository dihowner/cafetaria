...
const BankSetUp = () => {
    ...
    const { Allbanks, verifyBank, fetchBankLoading, saveBankDetails, saveBanKLoading, loading } = BanksFetch()
    ...
    useEffect(() => {
        if (bankDetails) {
            setBank_code(bankDetails?.bankId || ''); // assuming bank_code is a string
    ...
    return (
        <>
            ...
            {saveBanKLoading ? <LoaderTwo color={'#5f8357'} loading={saveBanKLoading} /> : null}
            <div className='w-full border p-4 flex flex-col gap-y-8 '>
                ...
                <form onSubmit={SaveBank} className="flex flex-col gap-y-4 text-sm">
                    ...
                    <div className="border w-[100%] sm:w-[80%] md:w-[80%] px-4 py-3 flex items-center gap-x-6 rounded-lg">
                        ...
                        <Autocomplete
                            value={options.find((option) => option.value === bank_code) || null}
                            onChange={(event, newValue) => setBank_code(newValue?.value || '')}
